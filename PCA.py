...
class pca_class:
    ...
    def give_p(self, d):
        ...
        for i in d:
            x_array+=[x]
            x+=1
        plt.plot(x_array,d,'ro-',linewidth=1)
        ...
        print(sum)
        print(d)
        print("sum of eigen values:",sum)
        sum_90 = self.quality_percent * sum/100
        print("90 % of sum:",sum_90)
        temp = 0
        p = 0
        while temp < sum_90:
            temp += d[p]
            p += 1
        print("Values of k or p",p)
        return p

    def reduce_dim(self):

        print("Original Shape:",self.images.shape)
        p, d, q = s_linalg.svd(self.images, full_matrices=True)
        print("p shape",p.shape)
        print(p)
        print("q shape",q.shape)
        print(q)
        print("Total eigen values:",d.shape)
        p_matrix = np.matrix(p)
        d_matrix = np.matrix(d)
